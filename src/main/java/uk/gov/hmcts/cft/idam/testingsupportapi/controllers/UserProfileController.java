...
public class UserProfileController {
    ...
    public UserProfileController(TestingSessionService testingSessionService, TestingUserProfileService testingUserProfileService) {
    ...
    public UserProfile getUserProfileById(@AuthenticationPrincipal @Parameter(hidden = true) Jwt principal, @PathVariable String userId) {
    ...
    @ResponseStatus(HttpStatus.OK)
    @SecurityRequirement(name = "bearerAuth")
    public User createOrUpdateCftUser(@AuthenticationPrincipal @Parameter(hidden = true) Jwt principal, @PathVariable String userId, @RequestBody ActivatedUserRequest request) throws Exception {
        TestingSession session = testingSessionService.getOrCreateSession(principal);
        return testingUserProfileService.createOrUpdateCftUser(session.getId(), request.getUser(), request.getPassword());
    }
